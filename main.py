...
while True:
    ...
    if conFound:
        ...
        # Speak the amount asynchronously
        string_mon = str(moneyAmt)
        if string_mon != '0' and previous_count != moneyAmt:
            thread = threading.Thread(target=speak_async, args=(string_mon + 'Pesos',))
            thread.start()
            previous_count = moneyAmt

        cv2.putText(moneyCount, f'Php{moneyAmt}', org, font, font_scale, color, thickness=2, lineType=cv2.LINE_AA)
        # show images
        imgStacked = cvzone.stackImages([img, imgBlur, imgEdge, imgContours, moneyCount], 3,
                                        1)  # good for showing the process
